...
def download_first_asset(
    product: dict[str, Any],
    out_path: Path,
    auth_headers: dict[str, str],
) -> Path:
    """Download the first available asset from a product feature.

    Used when the asset key is not known in advance, e.g. for single-file
    products where the STAC asset key is not standardised.
    """
    keys = list_asset_keys(product)
    if not keys:
        raise KeyError(f"No assets found for product {product.get('id')}")
    asset_key = keys[0]
    log.info(f"Downloading first available asset key: '{asset_key}'")
    return _download_asset(product, asset_key, out_path, auth_headers)
